...
def is_user_not_banned(message: Message) -> bool:
    return str(message.chat.id) not in databaseManager.get_blocked_users()


def is_admin_only(message: Message) -> bool:
    return (message.chat.id == AdminPanel.groupId.value) or (message.chat.id in adminIds)


def is_from_admin(message: Message) -> bool:
    return message.text.startswith('$') and (
        (
            message.chat.id == AdminPanel.groupId.value
            and message.message_thread_id == AdminPanel.commandPlaceThreadId.value
        )
        or message.chat.id in adminIds
    )


def is_admin_chat_and_notifier_thread(message: Message) -> bool:
    return message.chat.id == AdminPanel.groupId.value and message.message_thread_id in [
        thread.value for thread in Notifier
    ]


@bot.message_handler(commands=['start'], func=is_user_not_banned)
def start(message: Message) -> None:
    log(f'start call by {message.from_user.first_name}')
    bot.send_message(
        message.chat.id,
        'Привіт! Це бот, який автоматично слідкує за змінами у твоєму розкладі. 📅\n'
    ...
    fullName = (
        ...
        else f'{message.from_user.first_name} {message.from_user.last_name}'
    )

    databaseManager.save_user(message.from_user.id, fullName, message.from_user.username)

    if databaseManager.get_user_info(message.from_user.id)['group_name'] == '':
        change_group(message)


@bot.message_handler(commands=['change_group'], func=is_user_not_banned)
def change_group(message: Message) -> None:
    ...
    bot.send_message(
        message.chat.id,
        'Обери групу за розкладом якої ти хочеш слідкувати:',
...
def callback_handler(call: CallbackQuery) -> None:
    ...
    databaseManager.update_user_group(call.from_user.id, call.data)


@bot.message_handler(commands=['help'], func=is_user_not_banned)
def helpComm(message: Message) -> None:
...
@bot.message_handler(commands=['about'], func=is_user_not_banned)
def about(message: Message) -> None:
    log(f'about call by {message.from_user.first_name}')
    bot.send_message(
        message.chat.id,
        'Це бот для слідкування за змінами у розкладі.\n'
        'Ось як він працює:\n'
...
@bot.message_handler(commands=['schedule'], func=is_user_not_banned)
def schedule(message: Message) -> None:
    ...
    button1 = telebot.types.InlineKeyboardButton(
        ...
        web_app=telebot.types.WebAppInfo(
            url=f'https://remenod.github.io/ScheduleSpyBot/?currWeekGid={currWeekGid}'
        ),
    )
    button = telebot.types.InlineKeyboardButton(
        text='Відкрити розклад (URL)',
        url=f'https://docs.google.com/spreadsheets/u/0/d/{dataProcessor.SPREADSHEET_ID}'
...
@bot.message_handler(commands=['delete_week'], func=is_admin_only)
def delete_week(message: Message) -> None:
    log(f'delete_week call by {message.from_user.first_name}')
    try:
        cParts = message.text.split()
        if len(cParts) == 2:
            databaseManager.delete_sheet(cParts[1])
        else:
            bot.send_message(
                message.chat.id,
                'Будь ласка, вкажіть номер тижня. Наприклад: /delete_week 4',
                message_thread_id=message.message_thread_id,
            )
            return
        ...
        bot.send_message(
            message.chat.id,
            'Будь ласка, введіть коректний номер тижня. Наприклад: /delete_week 4',
            message_thread_id=message.message_thread_id,
        )
    except IndexError:
        bot.send_message(
            message.chat.id,
            'Немає тижня з таким номером. Перевірте ще раз.',
            message_thread_id=message.message_thread_id,
        )
...
@bot.message_handler(commands=['print'], func=is_admin_only)
def send_sheet_data(message: Message) -> None:
    ...
    try:
        ...
        if len(cParts) != 2:
            bot.send_message(
                message.chat.id,
                'Будь ласка, вкажіть номер аркуша. Наприклад: /print 4',
                message_thread_id=message.message_thread_id,
            )
            return
        ...
        bot.send_message(
            message.chat.id,
            dataProcessor.get_schedule(
                dataProcessor.load_workbook().worksheets[int(cParts[1]) - 1],
                Group.KC241_1,
                False,
            ),
            message_thread_id=message.message_thread_id,
        ...
        bot.send_message(
            message.chat.id,
            'Будь ласка, введіть коректний номер аркуша. Наприклад: /print 4',
            message_thread_id=message.message_thread_id,
        )
    except IndexError:
        bot.send_message(
...
@bot.message_handler(commands=['compare'], func=is_admin_only)
def compare(message: Message) -> None:
    ...
    try:
        ...
        workbook = dataProcessor.load_workbook()
        schedule1 = dataProcessor.get_schedule(workbook.worksheets[int(cParts[1]) - 1], Group.KN24_1)
        schedule2 = dataProcessor.get_schedule(workbook.worksheets[int(cParts[2]) - 1], Group.KN24_1)

        bot.send_message(message.chat.id, dataProcessor.compare_schedules(schedule1, schedule2))
...
@bot.message_handler(commands=['cool_compare'], func=is_admin_only)
def cool_compare(message: Message) -> None:
    ...
    try:
        ...
        if len(cParts) != 3:
            bot.send_message(
                message.chat.id,
                'Будь ласка, вкажіть два номери аркушів для порівняння. Наприклад: /cool_compare 4 5',
                message_thread_id=message.message_thread_id,
            )
            return
        ...
        workbook = dataProcessor.load_workbook()
        schedule1 = dataProcessor.get_schedule(workbook.worksheets[int(cParts[1]) - 1], Group.KC242_2)
        schedule2 = dataProcessor.get_schedule(workbook.worksheets[int(cParts[2]) - 1], Group.KC242_2)

        bot.send_message(
            message.chat.id,
            dataProcessor.parse_comparer_output(
                dataProcessor.compare_schedules(schedule1, schedule2)
            ),
            parse_mode='Markdown',
            message_thread_id=message.message_thread_id,
        )
...
@bot.message_handler(commands=['fill_schedule_table'], func=is_admin_only)
def fill_group_handler(message: Message) -> None:
    ...
    try:
        ...
        if len(cParts) != 2:
            bot.send_message(
                message.chat.id,
                'Будь ласка, вкажіть номер аркуша. Наприклад: /fill_schedule_table 4',
                message_thread_id=message.message_thread_id,
            )
            return
        log('Заповнюю бд...', message.message_thread_id)
        sheet = dataProcessor.load_workbook().worksheets[int(cParts[1]) - 1]
        sheetWeekNum = sheet.title.split('т')[0]
        for group in Group:
            log(f'Заповнюю групу {group.name}', message.message_thread_id)
            schedule = dataProcessor.get_schedule(sheet, group)
            databaseManager.write_schedule(sheetWeekNum, group, f'{schedule}')
        log('Готово', message.message_thread_id)

    except ValueError:
        bot.send_message(
            message.chat.id,
            'Будь ласка, введіть коректні номери аркушів для порівняння. Наприклад: /fill_schedule_table 4',
...
@bot.message_handler(commands=['call_checker'], func=is_admin_only)
def call_checker(message: Message) -> None:
    log('checker call')
    dataProcessor.compare_all_groups()


@bot.message_handler(commands=['stop'], func=is_admin_only)
def stop(message: Message) -> None:
    ...
    if current_system == 'Windows':
        ...
        try:
            log('Зупинка бота...')
            os._exit(0)
            os.system('taskkill /F /PID %d' % os.getpid())  # wtf
        except Exception as e:
            log(f'Помилка при спробі зупинки бота: {e}')
        log('Спроба зупинки бота на незареєстрованій системі завершилась невдачею.')


@bot.message_handler(commands=['ban'], func=is_admin_only)
def ban(message: Message) -> None:
    ...
    try:
        ...
        if len(cParts) != 2:
            bot.send_message(
                message.chat.id,
                'Будь ласка, вкажіть id або username. Наприклад: /ban @Telegram або /ban 1234567890',
                message_thread_id=message.message_thread_id,
            )
            return

        if cParts[1].startswith('@'):
            user = databaseManager.get_user_by_username(cParts[1])
            if user == {}:
                bot.send_message(
                    message.chat.id,
                    'Нема користувача з таким username.',
                    message_thread_id=message.message_thread_id,
                )
                return None
            databaseManager.block_user(user['chat_id'])
            bot.send_message(
                message.chat.id,
                f'Користувач {user["full_name"]}({user["username"]}) заблокований.',
                message_thread_id=message.message_thread_id,
            )

        elif cParts[1].isdigit():
            databaseManager.block_user(cParts[1])
            bot.send_message(
                message.chat.id,
                f'Користувач {cParts[1]} заблокований.',
                message_thread_id=message.message_thread_id,
            )
    except Exception as e:
        bot.send_message(
            message.chat.id,
            f'Сталася помилка блокування: {e}',
            message_thread_id=message.message_thread_id,
        )
...
def execMsg(message: Message) -> None:
    try:
        exec(message.text.replace('$', '', 1))
    except Exception as e:
        log(e)


@bot.message_handler(
    func=is_admin_chat_and_notifier_thread,
    content_types=[
        'text',
        'photo',
        'video',
...
def notify(message: Message) -> None:
    if Notifier(message.message_thread_id) != Notifier.general:
        ...
        users = databaseManager.get_users_by_group(group)
    else:
    ...
    if users:
        ...
        log(f'Повідомлення отримали:\n {success_users} ({len(success_users)}/{len(users)})')
    else:
        log('Сталася помилка. Групу для масового сповіщення не знайдено')
