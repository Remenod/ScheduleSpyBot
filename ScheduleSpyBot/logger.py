...
def log(text:str, threadId:int = AdminPanel.logerThreadId.value):    
    try:
        ...
        if(threadId == AdminPanel.logerThreadId.value):
            logClient.send_message(AdminPanel.groupId.value, str(text), message_thread_id=threadId)
        else:
            bot.send_message(AdminPanel.groupId.value, str(text), message_thread_id=threadId)
